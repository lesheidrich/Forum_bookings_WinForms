<?php
// bejelentkezés külön "endpoint"-ban történik
include("./db.php");
		...
		if (!empty($_GET["username"]) && !empty($_GET["password"])) {
			if (login($_GET["username"], $_GET["password"])) {
				echo json_encode(array(
                        ...
                        'isAdmin' => checkAdmin($_GET["username"], $_GET["password"])
						// ,'id' => checkId($_GET["username"], $_GET["password"])
					)
				);
			}
			else {
...
//     $result = $con -> query("SELECT id FROM users WHERE username = '$u' AND password = MD5('$p');") -> fetch_all();
//     return $result;
// }
